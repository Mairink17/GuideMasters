...
fun getDadosUsuarioLogado(callback: (Perfil?) -> Unit) {
    ...
    userId?.let { uid ->
        ...
        usuarioRef.get().addOnSuccessListener { documentSnapshot ->
            if (documentSnapshot.exists()) {
                ...
                val tipoPerfil = documentSnapshot.getString(Constantes.CAMPO_PERFIL) ?: ""
                var tipoPerfilAjustado: TipoPerfil = when (tipoPerfil) {
                    "MENTORADO" -> TipoPerfil.MENTORADO
                    else -> TipoPerfil.MENTOR
                }
